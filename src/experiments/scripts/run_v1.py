...
def main():
    ...
    train_transform = get_train_transform(False, args.dataset)
    train_augmentation_transform = get_train_transform(True, args.dataset)
    test_transform = get_test_transform(args.dataset)

    if args.dataset == 'cifar':
        train_random_dataset = CIFAR10RandomLabels(root=base_settings.DATA_ROOT,
                                                   corrupted_labels_path=base_settings.CIFAR_CORRUPTED_LABELS_PATH,
        ...
        train_true_dataset = MiniImageNet(train=True, transform=train_augmentation_transform)
