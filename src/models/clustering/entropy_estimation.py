...
class EntropyEstimator(latent_space_sampler.LatentSpaceSampler):
    ...
    def __init__(self, trace_pkl_path, samples_num, entropy_type, **kwargs):
        ...
        if entropy_type == 'relative' and 'data_trace_path' not in kwargs:
            raise ValueError("For entropy_type == 'relative' data_trace_path should be give in kwargs")

        if entropy_type == 'relative':
            self.data_trace_path = kwargs['data_trace_path']
    ...
    def __get_invariant_distr(self):
        data_trace = fs_utils.read_pickle(self.data_trace_path)
